...
final class LiveTranscriptionSession: NSObject, @unchecked Sendable {
    ...
    private let captureQueue = DispatchQueue(label: "com.lqy007700.hearsub.capture", qos: .userInitiated)
    ...
    private var applicationAudioCapture: AnyObject?
    ...
    private func stopOnCaptureQueue() {
        ...
        if #available(macOS 15.0, *) {
            (applicationAudioCapture as? ApplicationAudioCapture)?.stop()
        }
    ...
    private func makeApplicationCaptureDescriptor(for source: InputSource) throws -> ApplicationCaptureDescriptor {
        if source.category == .systemAudio {
            return ApplicationCaptureDescriptor(
                appName: source.name,
                processObjectIDs: [],
                capturesAllProcesses: true,
    ...
    private func startApplicationAudioCapture(descriptor: ApplicationCaptureDescriptor) throws {
        let capture = ApplicationAudioCapture(
            ...
            capturesAllProcesses: descriptor.capturesAllProcesses,
...
private final class ApplicationAudioCapture {
    ...
    private let capturesAllProcesses: Bool
    ...
    init(
        ...
        self.capturesAllProcesses = capturesAllProcesses
    ...
    func start() throws {
        do {
            let tapDescription = capturesAllProcesses
            ...
            tapDescription.name = "HearSub \(appName)"
            ...
            let aggregateDescription: [String: Any] = [
                kAudioAggregateDeviceNameKey: "HearSub-\(appName)",
