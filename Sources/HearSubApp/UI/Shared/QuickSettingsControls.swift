    ...
    var body: some View {
        Picker("", selection: $selection) {
            ...
            ForEach(sources) { source in
                Text(source.displayName(in: interfaceLanguageID))
...
private extension InputSource {
    func displayName(in languageID: String) -> String {
        if category == .systemAudio {
            return category.displayName(in: languageID)
        }

        return "\(category.displayName(in: languageID)) · \(name)"
    }
}
