...
final class AppModel: ObservableObject {
    ...
    @Published private(set) var openAICompatibleModelOptions: [String] = []
    @Published private(set) var isFetchingOpenAICompatibleModels = false
    @Published private(set) var openAICompatibleModelFetchError: String?
    @Published private(set) var isTestingOpenAICompatibleConnection = false
    @Published private(set) var openAICompatibleConnectionTestMessage: String?
    @Published private(set) var openAICompatibleConnectionTestSucceeded: Bool?
    ...
    @Published var openAICompatibleTranslation: OpenAICompatibleTranslationSettings {
        didSet {
            persistSettings()
            openAICompatibleModelFetchError = nil
            openAICompatibleConnectionTestMessage = nil
            openAICompatibleConnectionTestSucceeded = nil
            if translationProvider == .openAICompatible, liveTranscriptionSession != nil {
                refreshCaptionTranslations()
            }
        }
    }
    ...
    init(
        ...
        let persistedTranscript = transcriptStore.load()
    ...
    var isOpenAICompatibleTranslationConfigured: Bool {
        openAICompatibleTranslation.baseURL.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty == false
            && openAICompatibleTranslation.apiKey.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty == false
            && openAICompatibleTranslation.model.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty == false
    }

    var startupConfigurationWarnings: [String] {
        var warnings: [String] = []
        ...
        if showsTranslatedSubtitle {
            ...
            if openAICompatibleTranslation.model.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                warnings.append(localized(.modelNotConfigured))
            }
        }
    ...
    func scrollOverlayHistory(by delta: Int) {
        ...
        setOverlayHistoryScrollOffset(overlayHistoryScrollOffset + step)
    }
    ...
    private static func normalizedOverlayStyle(_ style: OverlayStyle) -> OverlayStyle {
        ...
        if normalized.widthRatio <= 0.68,
           ...
           normalized.translatedFontSize <= 18,
           normalized.sourceFontSize <= 12 {
            normalized.topInset = OverlayStyle.default.topInset
            normalized.widthRatio = OverlayStyle.default.widthRatio
            normalized.minWidth = OverlayStyle.default.minWidth
            normalized.maxWidth = OverlayStyle.default.maxWidth
            normalized.backgroundOpacity = OverlayStyle.default.backgroundOpacity
            normalized.translatedFontSize = OverlayStyle.default.translatedFontSize
            normalized.sourceFontSize = OverlayStyle.default.sourceFontSize
        }
        return normalized
    }
    ...
    func refreshOpenAICompatibleModels() {
        guard isFetchingOpenAICompatibleModels == false else { return }

        isFetchingOpenAICompatibleModels = true
        openAICompatibleModelFetchError = nil
        let settings = openAICompatibleTranslation

        Task { @MainActor [weak self] in
            guard let self else { return }
            defer { self.isFetchingOpenAICompatibleModels = false }

            do {
                let models = try await self.openAICompatibleTranslationService.fetchModels(settings: settings)
                self.openAICompatibleModelOptions = models

                let selectedModel = self.openAICompatibleTranslation.model
                    .trimmingCharacters(in: .whitespacesAndNewlines)
                if selectedModel.isEmpty, let firstModel = models.first {
                    var updatedSettings = self.openAICompatibleTranslation
                    updatedSettings.model = firstModel
                    self.openAICompatibleTranslation = updatedSettings
                }
            } catch {
                self.openAICompatibleModelOptions = []
                self.openAICompatibleModelFetchError = self.localizedErrorDescription(error)
            }
        }
    }

    func testOpenAICompatibleTranslationConnection() {
        guard isTestingOpenAICompatibleConnection == false else { return }

        isTestingOpenAICompatibleConnection = true
        openAICompatibleConnectionTestMessage = nil
        openAICompatibleConnectionTestSucceeded = nil
        let initialSettings = openAICompatibleTranslation

        Task { @MainActor [weak self] in
            guard let self else { return }
            defer { self.isTestingOpenAICompatibleConnection = false }
            ...
            do {
                ...
                _ = try await self.openAICompatibleTranslationService.translate(
                    "Hello",
                    from: "en",
                ...
                self.openAICompatibleConnectionTestMessage = self.localized(.connectionTestSucceeded)
            } catch {
                self.openAICompatibleConnectionTestSucceeded = false
                self.openAICompatibleConnectionTestMessage = self.localizedErrorDescription(error)
            }
        }
    }
    ...
    private func scheduleDraftTranslation(
        ...
        draftTranslationTask = Task { @MainActor [weak self] in
            ...
            let translated = await withTaskGroup(of: String?.self, returning: String?.self) { group in
                group.addTask {
                    try? await self.translateSubtitleText(
                        text,
    ...
    func deleteTranscriptEntry(id: UUID, from sessionID: UUID? = nil) {
        ...
        overlayState?.history.removeAll { $0.id == id }
    ...
    func clearOverlayHistory() {
        overlayState?.history.removeAll()
        overlayHistoryVisibleCount = 0
        overlayHistoryScrollOffset = 0
    }
    ...
    private func translateSubtitleText(
        _ text: String,
        from sourceLanguageID: String,
        to targetLanguageID: String
    ) async throws -> String {
    ...
    private func translatedText(for caption: QueuedCaption) async -> String? {
        ...
        let raw: String?
        do {
            raw = try await translateSubtitleText(
                caption.sourceText,
        ...
        let translated = sanitizedDisplayText(raw)
    ...
    private func trimTranscriptIfNeeded() {
        let overflow = max(0, transcriptEntries.count - Self.transcriptHistoryLimit)
    ...
    private var overlayHistoryMaxScrollOffset: Int {
        guard overlayHistoryCount > 0 else { return 0 }
        return max(1, overlayHistoryCount - max(0, overlayHistoryVisibleCount) + 1)
    }
    ...
    private func sampleText(for languageID: String) -> String {
        switch languageID {
        case "zh-Hans":
            return "欢迎使用 HearSub，顶部字幕条已经准备好了。"
        case "es":
            return "Bienvenido a HearSub. La barra de subtitulos ya esta lista."
        case "de":
            return "Willkommen bei HearSub. Die Untertitel-Leiste ist bereit."
        case "ja":
            return "HearSub へようこそ。字幕バーの準備ができました。"
        case "fr":
            return "Bienvenue dans HearSub. La barre de sous-titres est prete."
        case "it":
            return "Benvenuto in HearSub. La barra dei sottotitoli e pronta."
        case "ko":
            return "HearSub에 오신 것을 환영합니다. 자막 바가 준비되었습니다."
        case "yue":
            return "歡迎使用 HearSub，字幕列已經準備好。"
        case "ar":
            return "مرحبا بك في HearSub. شريط الترجمة جاهز."
        case "pt":
            return "Bem-vindo ao HearSub. A barra de legendas esta pronta."
        case "ru":
            return "Добро пожаловать в HearSub. Строка субтитров готова."
        default:
            return "Welcome to HearSub. The subtitle bar is ready."
        }
    }
...
@MainActor
extension View {
    func hearSubTranslationHost(model: AppModel) -> some View {
        _ = model
        return self
    }
}
